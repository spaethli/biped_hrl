...
adopted.** Stated reason: on hardware the DR arms did not come fully to rest at zero command
(residual motion, mainly arm swing, no unplanned steps), and `A0_DR` under the backpack did not
walk better than A1a. ⚠ **Narrowed by the 2026-09-16 seed band (`docs/adr/0013` amendment):** the
standing defect is seed 123 of both DR recipes, the seed-42 DR arms match A1a, and no hierarchical
DR arm was ever loaded on the robot. So "not adopted", not "DR breaks the policy". The DR arms are
kept, not deleted.
