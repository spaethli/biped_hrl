...
| **A1** | `Unitree-H1_2-Flat-A1` | ✅ **tracking wall SOLVED** (2026-06-16) | HIRO: PPO LL + TD3 HL + relabel. `absolute` target + `tracking` HL reward → **A0-level** err 0.098/0.091/0.167, 0 falls. Polish (yaw, smoothness) + writeup ablations remain. |
...
- **A1 design (as-built) + current status + open ablations** → `doc/A1_HIRO.md`.
...
changes (RQ2 rule). Each architecture gets a `doc/A<N>_*.md` design+status doc that links
