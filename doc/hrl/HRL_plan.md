...
| **A1** | `Unitree-H1_2-Flat-A1` | ✅ **tracking wall SOLVED** (2026-06-16) | HIRO: PPO LL + TD3 HL + relabel. `absolute` target + `tracking` HL reward → **A0-level** err 0.098/0.091/0.167, 0 falls. Sim deploy done (two-ONNX C++ `State_RLHRL`); sim2real next = state-noise DR (LL twitchy under injected estimator noise). Polish + ablations remain. |
