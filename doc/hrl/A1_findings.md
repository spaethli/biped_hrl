...
| **A1a stage D: arm calm + held-command root cause** (2026-07-14/15, D1 `2026-07-14_11-39-53_*shw16-4*_s42` W&B `qcxbn7yt`, D2 `*_shw16-4_*_rs20_s42`, A0 `a0_v2_optB_rs20_baseline`) | **Per-joint posture weights calm the arms (validated ×2)**; ~~held-command failure = TD3-HL velocity-hold degeneracy, isolated to the hierarchy~~ **[the hold half is VOID — eval artifact; see the WL-C row. Re-measured post-F2 (2026-07-16): D1/D2 are the BEST holders in the set (ss@1.0 0.039/0.042 vs A0 0.076) — the arm lever *improves* holds and the artifact had exactly inverted the ranking (D1's 1.033 was the old table's worst number)]** | Tables + reads → `A1a_plan.md` (f). Levers/fixes shipped: `ll_posture_weights` (shoulders 16 / elbow+wrist 4, LL intrinsic, weighted mean not renormalized, per-joint clamp), training `resampling_time_range` (3,8)→(3,20) with the bench pinned (3,8) (comparability preserved, keeper agg invariant 0.0797 vs 0.080), hold-eval `ss_err`/`t90` + `[HOLDDIAG]` per-env group diff. **Two eval-tooling bugs found+fixed**: (1) `hl_obs_vel` absence-shim missing in play.py structure restore → every pre-velobs TD3 checkpoint mis-built 94-dim HL nets vs saved 92 (same class as the `hl_velocity_goals_only` shim); (2) the `--diagnose-goals` loop never advanced `env.hrl_phase` → **all pre-2026-07-15 GOALDIAG numbers on cadence-HL checkpoints ran with a frozen LL phase clock (de-entrained ⇒ flattering) and 1 env unless `--num-envs` was passed** — table (d) probe reads carry that asterisk. Falsified for the hold failure: duration (rs20), heading, exact-zero vy/wz, clean-eval dither. Lessons: **batch-mean hold metrics hide bimodal modes — per-env split is mandatory**; the aggregate bench structurally cannot see absorbing modes (3–8 s resample breaks them); LL is exonerated again (`ll_err` 0.025). Next: verify HIRO-relabeling hold-bias suspect in `td3.py` before any new training arm. **[SUPERSEDED 2026-07-16 — the whole "velocity-hold degeneracy" was an eval artifact; see the WL-C row below.]** |
| **WL-C: the held-command "HL hold degeneracy" is an EVAL-TOOLING ARTIFACT (third of the family)** (2026-07-16, keeper `2026-07-09_10-04-50_*`, control `2026-07-16_11-06-48_*_norelabel_s42` W&B `vg8x3gjf`) | **There is no HL hold degeneracy. `--eval-cmd-vx` silently zeroes the A1 goal scale → the hierarchy's goal channel is inert during every hold eval. Fixing the scale (checkpoint untouched) makes the keeper hold as well as A0. HIRO-relabel suspect FALSIFIED.** | **Mechanism:** `play.py:297-299` collapses the twist ranges to a point (`lin_vel_x=(vx,vx)`) to pin the hold command; `goal_space.py:79-83` derives the HIRO goal scale from *those same live ranges* (`half=max((hi−lo)/2, 1e-3)`) → **scale [0.75,0.5,1.0] → [1e-3,1e-3,1e-3]**. So `V* = s_t + 1e-3·g ≈ s_t` for **any** `g`: the HL's action is disconnected from the LL, and the composed system is exactly the "driftless random walk" that was observed. Every documented symptom falls out of this one line and is reproduced/cured on the **unchanged** keeper: `\|g\|_vx` 0.98 sat 95.6% (the actor is screaming for max delta and getting 1e-3 of it) → **0.219** sat 4.6%; `hl_err_vx` 0.912 → **0.132**; `ll_err` 0.035 (LL "reaches" `V*≈s` trivially — that is why the LL kept getting exonerated); `goal_vx[k]≈ach_vx[k−1]` (literally `V*=s_t`) → `goal_vx` now *leads*; 38fwd/19bwd → **64/64 fwd** (the backwards absorbing mode was the random walk's coin flip). Bench, same ckpt: **ss@0.5 0.304→0.044 (t90 never→0.56 s), ss@1.0 0.882→0.097 (t90 never→0.76 s)** vs A0-optB-rs20 0.055/0.077 → **A1 holds ≥ A0**, falls 0. A0 is structurally immune (reads the command directly, no goal space) — which is exactly why the defect "isolated to the hierarchy" and why rs20/heading/zero-cmd/dither all failed to move it. **Relabel suspect falsified twice over**: (1) `Loss/hl/relabel_frac` = 0.79 at iter 0 (random warmup, expected) then **0.03–0.09 for iters 1000–10000** — the relabel keeps the stored goal 95–97% of the time; (2) instrumented `_relabel` on real keeper windows (`num_candidates=10`, 9/10 candidates drawn around `g_emp`): `\|g_stored−g_emp\|=0.51` (far apart, so low frac is *discriminative*, not a stored≈emp tie — killing the chicken-and-egg objection), `\|g\|_vx` 0.652→**0.621** after relabel (−4.6%, no shrink toward hold), and the **signed pull is preserved**: `corr(g_vx, cmd−achieved)` 0.827→**0.798**, `g_vx\|e>0.2` +0.828→+0.795. The HL pulls hard and correctly; the relabel does not rewrite goals toward achieved-consistent deltas. **Invalidated by this**: every A1 `ss@0.5`/`ss@1.0`/`[HOLDDIAG]` number in `A1a_plan.md` (e)/(f) (keeper 0.354/0.869, D1 0.376/1.033, D2 0.470/0.851) and the 2026-07-13 "sustained-command under-tracking is a deploy risk" read. A0 hold numbers stay valid. **ALL RE-MEASURED post-F2 (2026-07-16, tables (e)+(f) rewritten): every A1 run holds ≥ A0** (ss@0.5 0.030–0.043 vs A0 0.057; ss@1.0 0.039–0.097 vs 0.076; 0 falls; t90 all ≤ A0's), probes healthy (`|g|vx` 0.10–0.22 sat≈0, was 0.98/95%). **The ranking inverted**: D1/D2 (the arm-calm runs) went from the table's *worst* holders (0.376/1.033, 0.470/0.851) to its *best* (0.030/0.039, 0.034/0.042) — so the arm lever improves holds too, and the only surviving A1-vs-A0 gap is **energy, not tracking** (CoT 0.82–1.04 vs 0.537, arm_vel 0.37–0.58 vs 0.14) = WL-D's brief. Deploy-candidate implication: D1/D2 dominate the keeper on holds+arms+tracking, losing only CoT/stride (WL-B/the user's call). **rs20's rationale is void** (it was adopted to make "bad-transient loops reachable"; keep or revert is a separate call — D2 showed it costs A0 nothing). **Two further range-vs-scale defects found (NOT yet fixed, need approval)**: (a) aggregate bench runs `wz=(−0.5,0.5)` → `scale_yaw=0.5` vs training 1.0, so **every A1 aggregate `err_yaw` is measured with the HL's yaw goals halved** (vx/vy scales are correct → `err_vx`/`err_vy` valid); (b) **deploy `velocity_hrl/v0/params/deploy.yaml` has `ang_vel_z: [-0.5, 0.5]` → the deployed C++ `GoalSpace::scale` yaw = 0.5 vs 1.0 trained** — precisely the failure `goal_space.h`'s own header warns about ("ranges MUST equal the ranges the policy was trained with"); flagged to WL-B, deploy/ untouched here. **Root lesson: deriving the HL's action scale from a mutable env cfg couples policy semantics to eval/deploy knobs — the scale belongs in the checkpoint** (like the normalizer). **FIX SHIPPED 2026-07-16 (F2; the user approved "do fix 2, the others are deprecated")**: `GoalSpace.freeze_scale` + `HierarchicalRunner.save/load` bake+pin `goal_scale`, `learn()` un-pins so training keeps deriving live (curriculum-tracked ⇒ **no training-semantics change, no retrain**), ONNX metadata gains `goal_scale`, and `play.py` carries an absence shim pinning the TRAINED scale for pre-bake checkpoints (keeper/D1/D2) — the freeze lands at `load()`, *before* the `--eval-cmd-vx` mutation, so the range collapse can no longer reach the scale (F1 subsumed) and yaw returns to 1.0 for old+new ckpts (the aggregate `err_yaw` bias is gone; F3 subsumed in sim). Verified end-to-end on the keeper via the real `play.py` (`[SHIM]` pins `[0.75,0.5,1.0,1.0,1.0,1.0,0.2]`): **ss@0.5 0.304→0.0436 (t90 never→0.56 s), ss@1.0 0.882→0.0979 (never→0.76 s)** (reproducing the monkeypatch 0.0438/0.0972), and the aggregate moves **only in yaw** — err_vx 0.0808→0.0798, err_vy 0.0774→0.0746 (invariant, their scales were already correct), **err_yaw 0.2344→0.1953** (the halved-yaw bias removed) — exactly the predicted mechanism, an independent check on the diagnosis. **A0 byte-clean** (no shim, ss@1.0 0.0779 = documented 0.077); fresh smoke bakes the measured training scale, evals via the baked path, exports it to ONNX; resume re-bakes live. **Remaining (WL-B, deploy/ untouched here): C++ `hrl::GoalSpace::scale()` still derives from `deploy.yaml` ranges (`ang_vel_z [-0.5,0.5]` ⇒ yaw 0.5 vs 1.0 trained, a live deploy defect) — switch it to the ONNX `goal_scale` metadata now exported.** **Relabel-off control** (`--agent.relabeling none`, otherwise keeper-identical incl. the keeper's `resampling_time_range` (3,8) — today's default is (3,20), which would have confounded it), reframed to a structure ablation since the blocker it was meant to test does not exist: agg err_vx 0.065 / err_yaw 0.150 / CoT 0.933 / stride **0.357** (at the 0.35 cadence floor), holds (scale-fixed) ss@0.5 **0.030** (t90 0.35 s) / ss@1.0 **0.069** (t90 0.70 s) vs keeper 0.081/0.234/0.815/0.615 and 0.044/0.097. **1 seed each — not a "relabel-off is better" claim** (seed-luck band, cf. the posture row); the load-bearing point is that **both hold fine ≈ A0**, exactly as the falsification predicts. With `relabel_frac`≈0.03–0.09, HIRO relabeling is a near-inert component of the "final A1 structure" — reportable, but dropping it is a structure decision for the user, not WL-C's call (needs a 2nd seed). |
